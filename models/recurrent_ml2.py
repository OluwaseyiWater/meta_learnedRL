...
class RecurrentPolicyNetwork(hk.Module):
    ...
    def __call__(self, obs, hidden_state):
        ...
        return tfd.Categorical(logits=logits), new_hidden_state

def recurrent_policy_fn(obs, hidden_state, num_bs, num_bands, num_power_levels):
    policy = RecurrentPolicyNetwork(num_bs, num_bands, num_power_levels)
    return policy(obs, hidden_state)

# Value network remains the same (non-recurrent for simplicity)
...
def make_recurrent_networks(num_bs, num_bands, num_power_levels):
...
#     # Inner adaptation (PPO updates + hidden state)
#     adapted_policy_params, adapted_value_params, adapted_hidden_state = inner_adaptation(
#         policy_params, value_params, policy_apply, value_apply, train_traj_prepared, inner_lr, inner_steps
#     )
    
#     # Sample test trajectories with adapted parameters and hidden state
#     test_traj = sample_trajectories(
#         task_env, adapted_policy_params, adapted_value_params, policy_apply, value_apply, test_key,
#         obs_norm_state, reward_norm_state, initial_hidden_state=adapted_hidden_state
#     )
...
#         adapted_policy_params, adapted_value_params, policy_apply, value_apply, test_traj_prepared
#     )
...
def compute_meta_objective_for_task(
    ...
    # Inner adaptation (via hidden state)
    adapted_policy_params, adapted_value_params, adapted_hidden_state = inner_adaptation(
        policy_params, value_params, policy_apply, value_apply, train_traj_prepared, inner_lr, inner_steps
    )
    
    # Sample test trajectories with adapted hidden state
    test_traj = sample_trajectories(
        task_env, adapted_policy_params, adapted_value_params, policy_apply, value_apply, test_key,
        obs_norm_state, reward_norm_state, initial_hidden_state=adapted_hidden_state
    )
...
# Instead of a class, use a tuple to hold the state
...
def compute_inner_loss(policy_params, value_params, policy_apply, value_apply, traj):
    ...
    action_dist, _ = policy_apply(policy_params, obs, None)
...
def compute_ppo_loss(
    policy_params, value_params, policy_apply, value_apply, traj, old_policy_params, clip_eps=0.2, entropy_coeff=0.01
):
    obs = traj['observations']  # Shape: (num_steps, obs_dim)
    actions = traj['actions']   # Shape: (num_steps, action_dim)
    advantages = traj['advantages']  # Shape: (num_steps,)
    returns = traj['returns']   # Shape: (num_steps,)
    
    # Initialize hidden states
    hidden_state = None
    old_hidden_state = None
    
    # Lists to store log probabilities and entropies
    new_log_probs_list = []
    old_log_probs_list = []
    entropy_list = []
    ...
    for t in range(obs.shape[0]):
        obs_t = obs[t]  # Shape: (obs_dim,)
        action_t = actions[t]  # Shape: (action_dim,)
        
        # Compute new policy probabilities
        action_dist, hidden_state = policy_apply(policy_params, obs_t, hidden_state)
        new_log_prob = action_dist.log_prob(action_t)
        entropy = action_dist.entropy()
        
        # Compute old policy probabilities
        old_action_dist, old_hidden_state = policy_apply(old_policy_params, obs_t, old_hidden_state)
        old_log_prob = old_action_dist.log_prob(action_t)
    ...
    # Stack results
    new_log_probs = jnp.stack(new_log_probs_list)  # Shape: (num_steps,)
    old_log_probs = jnp.stack(old_log_probs_list)  # Shape: (num_steps,)
    entropy = jnp.stack(entropy_list)  # Shape: (num_steps,)
    
    # Compute policy ratio
    ratio = jnp.exp(new_log_probs - old_log_probs)
    
    # PPO clipped objective
    surrogate1 = ratio * advantages[:, None]
    surrogate2 = jnp.clip(ratio, 1.0 - clip_eps, 1.0 + clip_eps) * advantages[:, None]
    policy_loss = -jnp.mean(jnp.minimum(surrogate1, surrogate2))
    
    # Value loss (can process all observations at once since value_apply is not recurrent)
    values = value_apply(value_params, obs)
    values = jnp.nan_to_num(values, nan=0.0)
    value_loss = jnp.mean(jnp.square(returns - values))
    
    # Entropy bonus
    entropy_bonus = entropy_coeff * jnp.mean(entropy)
    
    # Combined loss
    combined_loss = policy_loss + 0.5 * value_loss - entropy_bonus
...
def inner_adaptation(policy_params, value_params, policy_apply, value_apply, traj, inner_lr, inner_steps):
    prepared_traj = prepare_trajectory(traj, value_params, value_apply)
    final_hidden_state = traj['final_hidden_state']
    
    def ppo_step(step, curr_params):
        curr_policy_params, curr_value_params = curr_params
        grad_fn = jax.value_and_grad(lambda p: compute_ppo_loss(
            p[0], p[1], policy_apply, value_apply, prepared_traj, policy_params, clip_eps=0.2
        )[0], has_aux=False)
        
        ppo_loss, grads = grad_fn((curr_policy_params, curr_value_params))
        policy_grads, value_grads = grads
    ...
    adapted_params = (policy_params, value_params)
    for i in range(inner_steps):
        adapted_params = ppo_step(i, adapted_params)
    
    adapted_policy_params, adapted_value_params = adapted_params
    return adapted_policy_params, adapted_value_params, final_hidden_state
...
def train_maml(
    ...
    for iteration in range(num_iterations):
        ...
        avg_meta_loss = meta_loss_sum / num_tasks
        avg_grads = jax.tree.map(lambda g: g / num_tasks, all_grads)
        
        # Apply updates with gradient clipping for stability
        updates, opt_state = optimizer.update(avg_grads, opt_state)
        params = optax.apply_updates(params, updates)
        
        # Update normalizers periodically
        if iteration % 5 == 0:
            # Sample a batch of observations for normalizer update
...
def sample_trajectories(
    ...
    # Use provided initial hidden state, or let policy initialize it
    ...
    for _ in range(num_steps):
        ...
        # Get action distribution and updated hidden state from policy
        action_dist, hidden_state = policy_apply(policy_params, norm_obs, hidden_state)
...
def train_maml(
    ...
    # Initialize optimizer
    optimizer = optax.chain(
        optax.clip(1.0),  # Clip meta-gradients
        optax.adam(meta_lr)
    )
    opt_state = optimizer.init(params)
