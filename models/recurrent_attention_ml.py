...
def update_obs_normalizer(state, obs_batch): return state # Placeholder
def normalize_obs(state, obs): return (obs - state[0]) / jnp.sqrt(state[1] + state[2])

class RobustRecurrentActor(hk.Module):
    def __init__(self, lstm_layers: int, lstm_hidden_dim: int, mlp_hidden_dim: int, num_outputs: int, num_attention_heads: int):
        super().__init__()
        self.num_attention_heads = num_attention_heads
        self.attention = hk.MultiHeadAttention(num_heads=num_attention_heads, key_size=64, w_init_scale=2.)
        self.rnn = hk.DeepRNN([hk.LSTM(lstm_hidden_dim) for _ in range(lstm_layers)])
        self.body = hk.nets.MLP([mlp_hidden_dim, mlp_hidden_dim])
        self.head = hk.Linear(num_outputs)

    def __call__(self, obs: chex.Array, state: Optional[Sequence[hk.LSTMState]]) -> Tuple[chex.Array, Sequence[hk.LSTMState]]:
        batch_size = obs.shape[0]
        attention_input = obs[:, None, :]
        attended_obs = self.attention(attention_input, attention_input, attention_input).squeeze(axis=1)
        if state is None:
            state = self.rnn.initial_state(batch_size)
        embedding, new_state = self.rnn(attended_obs, state)
...
class Critic(hk.Module):
    def __init__(self, hidden_dim=128):
        super().__init__()
        self.body = hk.nets.MLP([hidden_dim, hidden_dim, hidden_dim])
        self.head = hk.Linear(1)
    def __call__(self, obs: chex.Array) -> chex.Array:
        return self.head(self.body(hk.Flatten()(obs))).squeeze(axis=-1)

def make_robust_recurrent_networks(num_outputs: int, config: Any):
    def actor_fn(obs, state):
        net = RobustRecurrentActor(
...
def sample_recurrent_trajectories(
    ...
    def scan_fn(carry, _):
        ...
        env_metrics = _get_env_metrics(next_state, env)
...
def compute_ppo_loss(
    ...
    loss_info = {
        ...
        "mean_sinr_violations": jnp.mean(traj["sinr_violations"]), "mean_latency_violations": jnp.mean(traj["latency_violations"])
    }
    return combined_loss, loss_info

### OPTIMIZATION: `evaluate_adaptation` is now cleaner ###
...
def evaluate_adaptation(
    ...
    """
    JIT-compiled and vmapped evaluation.
    This function performs a full pre- and post-adaptation evaluation for a *single* task.
    We will then vmap it over multiple task keys.
    """
    ...
    def ppo_inner_update(inner_p, inner_key):
        ...
        # We can reuse the pre-adaptation trajectory for the first gradient step,
        # but for simplicity and correctness (in case the inner loop logic changes),
        # we sample new trajectories.
...
def train_recurrent_maml_ppo(config: Dict) -> Tuple[Any, Dict]:
    ...
    actor, critic = make_robust_recurrent_networks(num_outputs, h_config)
    ...
    for i in range(h_config.num_meta_iters):
        ...
        if i % h_config.eval_interval == 0:
            ...
            # Vmap the entire evaluation function over the evaluation task keys
            ...
            # Average the metrics across the evaluation batch
            ...
            eval_summary = {
                "avg_pre_reward": avg_pre_metrics["reward"],
                "avg_pre_sinr_violations": avg_pre_metrics["sinr_violations"],
                "avg_pre_latency_violations": avg_pre_metrics["latency_violations"],
                "avg_post_reward": avg_post_metrics["reward"],
                "avg_post_sinr_violations": avg_post_metrics["sinr_violations"],
                "avg_post_latency_violations": avg_post_metrics["latency_violations"],
                "reward_improvement": avg_post_metrics["reward"] - avg_pre_metrics["reward"],
                "sinr_improvement": avg_pre_metrics["sinr_violations"] - avg_post_metrics["sinr_violations"],
                "latency_improvement": avg_pre_metrics["latency_violations"] - avg_post_metrics["latency_violations"],
            }
...
if __name__ == '__main__':
    config = {
        "seed": 42, "meta_lr": 2.5e-5, "inner_lr": 1e-4, "inner_steps": 5, "meta_batch_size": 16,
        "num_meta_iters": 500, "rollout_length": 128, "vf_coef": 0.5, "ent_coef": 0.01,
        "clip_epsilon": 0.2, "max_grad_norm": 0.5, "use_wandb": False, "gamma": 0.99, "lambda_": 0.95,
        "wandb_project": "maml-spectrum-access", "wandb_name": "robust-recurrent-maml-ppo-v3-optimized",
        "log_interval": 10, "eval_interval": 50, "num_eval_tasks": 10,
        "num_lstm_layers": 2, "lstm_hidden_dim": 128, "mlp_hidden_dim": 128, "num_attention_heads": 4
    }
